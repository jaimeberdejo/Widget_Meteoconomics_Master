...
def download_bienes_agregado() -> pd.DataFrame:
    ...
    df_work = df[
        [reporter_col, product_col, flow_col, "TIME_PERIOD", "OBS_VALUE"]
    ].copy()
    ...
    df_work["flow_type"] = (
        df_work["flow_code"]
        .astype(str)
        .map({"1": "importaciones", "2": "exportaciones"})
    )
    ...
    print(
        f"  Guardado: {FILE_BIENES_AGREGADO} ({len(df_pivot):,} filas, {FILE_BIENES_AGREGADO.stat().st_size / 1024 / 1024:.1f} MB)"
    )
    return df_pivot
...
def download_bienes_socios() -> pd.DataFrame:
    ...
    df_work = df[
        [reporter_col, partner_col, flow_col, "TIME_PERIOD", "OBS_VALUE"]
    ].copy()
    ...
    # Pivotar flow
    df_work["flow_type"] = (
        df_work["flow_code"].astype(str).map({"1": "imp_bienes", "2": "exp_bienes"})
    )
    df_work = df_work.dropna(subset=["flow_type"])
...
def prepare_comercio_socios(df_goods: pd.DataFrame) -> pd.DataFrame:
    ...
    df_merged["socio"] = (
        df_merged["socio_code"].map(SOCIOS_NOMBRES).fillna(df_merged["socio_code"])
    )
    ...
    print(
        f"  Guardado: {FILE_COMERCIO_SOCIOS} ({len(df_final):,} filas, {FILE_COMERCIO_SOCIOS.stat().st_size / 1024 / 1024:.1f} MB)"
    )
    return df_final
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force", action="store_true", help="Forzar descarga ignorando cache"
    )
